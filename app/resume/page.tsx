...
export default function ResumePage() {
  ...
  // Listen for the "open-resume-editor" event from Navbar's V logo menu
  useEffect(() => {
  ...
  const handlePasswordSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (passwordInput === resumeData.password) {
  ...
  return (
    <main className="min-h-screen pt-24 pb-10 px-4 flex flex-col items-center justify-center relative z-10">
      {/* TOP ACTION BAR - Print option only (Visible after password) */}
      ...
      <div className="printable-resume relative w-full max-w-4xl bg-white text-slate-800 rounded-3xl shadow-2xl overflow-hidden border border-slate-200">
        ...
        <div
          className={`grid grid-cols-1 md:grid-cols-12 min-h-[1000px] ${!isAuthenticated ? "filter blur-md select-none pointer-events-none" : ""}`}
        >
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2.5 text-xs text-slate-700 px-1">
                <p className="flex items-center gap-3">
                  <Phone size={14} className="text-[#a87068]" />{" "}
                  {resumeData.phone}
                </p>
                <p className="flex items-center gap-3">
                  <Mail size={14} className="text-[#a87068]" />{" "}
                  {resumeData.email}
                </p>
                <p className="flex items-center gap-3">
                  <Globe size={14} className="text-[#a87068]" />{" "}
                  {resumeData.website}
                </p>
                <p className="flex items-center gap-3">
                  <MapPin size={14} className="text-[#a87068]" />{" "}
                  {resumeData.address}
                </p>
              </div>
            </div>
              ...
              <div className="space-y-4 px-1">
                {resumeData.education.map((item, index) => (
                  <div key={index} className="space-y-0.5">
                    ...
                    <p className="text-[11px] text-slate-600">{item.school}</p>
                    <p className="text-[10px] text-slate-400">{item.period}</p>
                  </div>
                ))}
              </div>
